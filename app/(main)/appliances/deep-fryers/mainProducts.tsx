...
import DeepBar1 from "./components/bar/bar1";
import DeepBar3 from "./components/bar/bar3";
import DeepBar2 from "./components/bar/bar2";
import DeepProductsList from "./components/deepProductList";

const ITEMS_PER_PAGE = 40; // Number of products to display per page
export default function DeepFryerProducts() {
    ...
    async function fetchProducts() {
      try {
        const response = await fetch(
          "/api/name?page=1&limit=40&name=Deep Fryers"
        );
    ...
    <div>
      <div className="w-[880px] h-auto min-h-[1800px] rounded-md bg-white relative pb-8">
        <div className="w-full h-[60px] ">
          <DeepBar1 totalProducts={totalProducts} />
        </div>
