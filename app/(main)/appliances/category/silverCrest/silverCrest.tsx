...
import SilverCrestCard from "./silverCrestCard";
...
const fetchProducts = async (): Promise<Product[]> => {
  const response = await fetch(`/api/appliances/category/silverCrest`);
...
export default function SilverCrest() {
            ...
            >
              {products.map((product) => (
                <SwiperSlide key={product.id}>
                  <SilverCrestCard key={product.id} product={product} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
