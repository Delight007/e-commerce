...
import MicrowaveBar1 from "./components/bar/bar1";
import MicrowaveBar3 from "./components/bar/bar3";
import MicrowaveBar2 from "./components/bar/bar2";
import MicrowaveProductsList from "./components/microwaveProductList";

const ITEMS_PER_PAGE = 40; // Number of products to display per page
export default function MicrowaveProducts() {
    ...
    async function fetchProducts() {
      try {
        const response = await fetch(
          "/api/name?page=1&limit=40&name=Microwave"
        );
    ...
    <div>
      <div className="w-[880px] h-auto min-h-[1800px] rounded-md bg-white relative pb-8">
        <div className="w-full h-[60px] ">
          <MicrowaveBar1 totalProducts={totalProducts} />
        </div>
