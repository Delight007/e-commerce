...
import DishProductsList from "./components/dishProductList";
import RiceBar1 from "./components/bar/bar1";
import RiceBar2 from "./components/bar/bar2";
import RiceBar3 from "./components/bar/bar3";
import RiceProductsList from "./components/dishProductList";

const ITEMS_PER_PAGE = 40; // Number of products to display per page
export default function RiceCookerProducts() {
    ...
    async function fetchProducts() {
      try {
        const response = await fetch(
          "/api/name?page=1&limit=40&name=Rice Cookers"
        );
    ...
    <div>
      <div className="w-[880px] h-auto min-h-[1800px] rounded-md bg-white relative pb-8">
        <div className="w-full h-[60px] ">
          <RiceBar1 totalProducts={totalProducts} />
        </div>
