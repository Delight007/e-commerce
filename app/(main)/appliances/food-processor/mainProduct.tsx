...
import FoodProcessorBar1 from "./components/bar/bar1";
import FoodProcessorBar3 from "./components/bar/bar3";
import FoodprocessorBar2 from "./components/bar/bar2";
import FoodProcessorProductsList from "./components/fpProductsList";

const ITEMS_PER_PAGE = 40; // Number of products to display per page
export default function FoodProcessorProducts() {
    ...
    async function fetchProducts() {
      try {
        const response = await fetch(
          "/api/name?page=1&limit=40&name=Food Processor"
        );
    ...
    <div>
      <div className="w-[880px] h-auto min-h-[1800px] rounded-md bg-white relative pb-8">
        <div className="w-full h-[60px] ">
          <FoodProcessorBar1 totalProducts={totalProducts} />
        </div>
