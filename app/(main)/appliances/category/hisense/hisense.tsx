...
import HisenseCard from "./hisenseCard";
...
const fetchProducts = async (): Promise<Product[]> => {
  const response = await fetch(`/api/appliances/category/hisense`);
  if (!response.ok) {
            ...
            >
              {products.map((product) => (
                <SwiperSlide key={product.id}>
                  <HisenseCard key={product.id} product={product} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
