...
import EcoflowProductCard from "./ecoflowProductCard";
...
const EcoflowProductsList = ({
  ...
  const {
    ...
    queryKey: ["EcoflowBrand-products", page, brand], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <EcoflowProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default EcoflowProductsList;
