...
import SyinixCard from "./syinixCard";
...
const fetchProducts = async (): Promise<Product[]> => {
  const response = await fetch(`/api/appliances/category/syinix`);
  if (!response.ok) {
...
export default function Syinix() {
  const { data: products } = useQuery<Product[]>({
    queryKey: ["syinix-products"],
            ...
            >
              {products.map((product) => (
                <SwiperSlide key={product.id}>
                  <SyinixCard key={product.id} product={product} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
