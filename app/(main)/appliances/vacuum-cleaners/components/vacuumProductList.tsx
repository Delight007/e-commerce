...
import BlenderProductCard from "./vacuumProductCard";
import DishProductCard from "./vacuumProductCard";
...
const VacuumProductsList = ({
  ...
  const {
    ...
    queryKey: ["vacuum", page, name], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <VacuumProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default VacuumProductsList;
