...
const NexusProductsList = ({
  ...
  const {
    ...
    queryKey: ["NexusBrand-products", page, brand], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <NexusProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default NexusProductsList;
