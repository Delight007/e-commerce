...
import HisenseProductCard from "./hisenseProductCard";
...
const HisenseProductsList = ({
  ...
  const {
    ...
    queryKey: ["HisenseBrand-products", page, brand], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <HisenseProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default HisenseProductsList;
