...
import BlenderProductCard from "./blenderProductCard";
...
const BlenderProductsList = ({
  ...
  const {
    ...
    queryKey: ["Blenders", page, name], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <BlenderProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default BlenderProductsList;
