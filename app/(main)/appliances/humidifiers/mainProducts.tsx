...
import HumidifierProductsList from "./components/humidifierProductList";

const ITEMS_PER_PAGE = 40; // Number of products to display per page
export default function HumidifierProducts() {
    ...
    async function fetchProducts() {
      try {
        const response = await fetch(
          "/api/name?page=1&limit=40&name=Humidifiers"
        );
    ...
    <div>
      <div className="w-[880px] h-auto min-h-[1800px] rounded-md bg-white relative pb-8">
        <div className="w-full h-[60px] ">
          <HumidifierBar1 totalProducts={totalProducts} />
        </div>
