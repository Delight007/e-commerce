...
import DishProductCard from "./deepProductCard";
import DeepProductCard from "./deepProductCard";
...
const DeepProductsList = ({
  ...
  const {
    ...
    queryKey: ["deep fryers", page, name], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <DeepProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default DeepProductsList;
