...
import DishProductCard from "./airProductCard";
import DeepProductCard from "./airProductCard";
...
const AirProductsList = ({
  ...
  const {
    ...
    queryKey: ["air fryers", page, name], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <AirProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};

export default AirProductsList;
