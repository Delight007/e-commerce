...
import RiceProductCard from "./dishProductCard";
...
const RiceProductsList = ({
  ...
  const {
    ...
    queryKey: ["Rice Cookers", page, name], // Include category in the query key
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
      {productsData?.products?.map((product: any) => (
        <RiceProductCard key={product.id} product={product} />
      ))}
    </div>
  );
};
