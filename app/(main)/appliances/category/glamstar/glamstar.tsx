...
import GlamstarCard from "./glamstarCard";
...
const fetchProducts = async (): Promise<Product[]> => {
  const response = await fetch(`/api/appliances/category/glamstar`);
...
export default function Glamstar() {
  const { data: products } = useQuery<Product[]>({
    queryKey: ["glamstar-products"],
            ...
            >
              {products.map((product) => (
                <SwiperSlide key={product.id}>
                  <GlamstarCard key={product.id} product={product} />
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
