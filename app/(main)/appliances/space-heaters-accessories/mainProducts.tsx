...
import HeaterBar1 from "./components/bar/bar1";
import HeaterhBar3 from "./components/bar/bar3";
import HeaterBar2 from "./components/bar/bar2";
import HeaterProductsList from "./components/heaterProductList";

const ITEMS_PER_PAGE = 40; // Number of products to display per page
export default function HeaterProducts() {
  // Number of products found based on the brand
  const [totalProducts, setTotalProducts] = useState<number | null>(null);
  useEffect(() => {
    async function fetchProducts() {
      try {
        const response = await fetch("/api/name?page=1&limit=40&name=Heaters");
    ...
    <div>
      <div className="w-[880px] h-auto min-h-[1800px] rounded-md bg-white relative pb-8">
        <div className="w-full h-[60px] ">
          <HeaterBar1 totalProducts={totalProducts} />
        </div>
